...
//导航跳转页面
    public function navgoods() {
        ...
        //分类查询
        $cate = M('category')->where("group_id='$nid' and ctype=0 and path=0")->select(); //一级分类
        ...
        //该导航页面相关品牌的查询
        $brand1 = M('brand')->where("group_id='$nid' and fz_id='$fz'")->field("brand_id,brand_name,brand_logo,site_url")->order("sort desc")->limit(0, 5)->select();
        $this->assign('brand1', $brand1);
        $brand2 = M('brand')->where("group_id='$nid' and fz_id='$fz'")->field("brand_id,brand_name,brand_logo,site_url")->order("sort desc")->limit(5, 10)->select();
        $this->assign('brand2', $brand2);
        $brand3 = M('brand')->where("group_id='$nid' and fz_id='$fz'")->field("brand_id,brand_name,brand_logo,site_url")->order("sort desc")->limit(10, 15)->select();
        ...
        $newgoods1 = M('goods')->where("is_new = 1 and fz_id='$fz'")->where($brand)->order('sort desc')->field("lx_goods.goods_id,goods_name,goods_thumb")->limit(1)->select();
        $newgoods = M('goods')->where("is_new = 1 and fz_id='$fz'")->where($brand)->order('sort desc')->field("lx_goods.goods_id,goods_name,goods_thumb")->limit(1, 5)->select();

        $this->assign('newgoods1', $newgoods1);
        $this->assign('newgoods', $newgoods);

        //导航页面楼层展示
        $cate = M('category')->where("group_id='$nid' and path=0")->field("cat_id")->select(); //一级分类
        if (!$cate) {
        ...
        $cate1 = M('category')->where($parent)->order('sort asc')->limit(4)->select(); //二级分类
        if (!$cate1) {
            $this->redirect("Public/error1");
        }
        ...
        foreach ($cate1 as $k => $v) {
            ...
            foreach ($arr[$cname]['cat'] as $i => $j) {
                $cid = $j['cat_id'];
                $ccc = M('goods')->where("fz_id='$fz'")->join('lx_category ON lx_goods.cat_id=lx_category.cat_id', 'left')->where("lx_category.pid='$cid'")->field("goods_id,goods_name,brand_id,goods_thumb")->select();
                // var_dump($ccc);
            ...
            for ($i = 0; $i < $g_count; $i++) {
                $gid = $good[$i];
                $good2[$i] = M('goods')->join('lx_goods_attr ON lx_goods.goods_id=lx_goods_attr.goods_id', 'left')->where("lx_goods.goods_id='$gid' and lx_goods.fz_id='$fz'")->field('lx_goods.goods_id,goods_name,goods_thumb,lx_goods_attr.market_price')->find();
            }
