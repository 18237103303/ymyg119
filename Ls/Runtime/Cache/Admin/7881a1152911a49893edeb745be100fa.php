  ...
  <div class="row">
				<div class="col-sm-12">
					...
					<div class="panel panel-default">
						...
						<form action="<?php echo U('Comment/comment');?>" method="post">
							<div class="col-sm-5">
			...
			<div class="panel panel-default">
				...
				<form action=<?php echo U('Words/delall');?> method="post" id="yourformid">
				<div class="panel-body">
					<table class="table table-bordered table-striped" id="example-2">
						<thead>
							...
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tbody class="middle-align">
					
							<tr>
							<td><input type="checkbox"  name="cName[]" value="<?php echo ($v['comment_id']); ?>"></td>
								<td><?php echo ($v["user_name"]); ?></td>
								<?php $id=$v['goods_id']; $rs=M('goods')->where("goods_id='$id'")->find(); ?>
								<td><?php echo ($rs["goods_name"]); ?></td>
								<td><?php echo ($v["comment_rank"]); ?>星级</td>
								<td><?php echo ($v["goods_id"]); ?></td>
								<td><?php echo ($v["com_ip"]); ?></td>
								<td><?php echo ($v["com_time"]); ?></td>
								<!-- <td><a href="<?php echo ($v["nav_url"]); ?>" target="_blank"><?php echo mb_substr($v['content'],0,50,'utf-8'); ?></a></td> -->
								<td>
								 	 <a href="<?php echo U('Comment/detail',array('id'=>$v['comment_id']));?>" class="btn btn-secondary btn-sm btn-icon icon-left">
										查看详情
									</a> 
									<!-- <a href="<?php echo U('words/edit',array('id'=>$v['id']));?>" class="btn btn-secondary btn-sm btn-icon icon-left">
										已查看留言
									</a>  -->
									<a href="<?php echo U('Comment/delete',array('id'=>$v['comment_id']));?>" class="btn btn-danger btn-sm btn-icon icon-left" onclick="return confirm('确定要删除吗？');">
										删除
									</a>

								</td>
							</tr>
							<tr><td colspan="6"><?php echo ($pages); ?></td></tr>
...
<!--
 <script type="text/javascript">
	$("#commentform").submit(function(e){
		...
		e.preventDefault();
		//2.ajax提交数据
		$.post('<?php echo U("Nav/navsort");?>',{'id':$("#id").val(),'sort':$("#sort").val()},function(data){
