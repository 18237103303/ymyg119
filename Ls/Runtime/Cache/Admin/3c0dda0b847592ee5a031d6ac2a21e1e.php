					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							<div class="col-sm-7">
								...
								<a class="btn btn-turquoise"  id="add_cate" href="<?php echo U('Sys/link_add');?>">新增友情链接</a>
                                                        </div>
						</div>
					</div>	
				</div>
					...
					<table class="table table-bordered table-striped" id="example-2">
						...
						<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									...
									<a href="<?php echo U('Sys/link_edit');?>?id=<?php echo ($vo["link_id"]); ?>" class="btn btn-secondary btn-sm btn-icon icon-left" >
										编辑
									</a>
									
									<a href="<?php echo U('Sys/delete_link');?>?id=<?php echo ($vo["link_id"]); ?>" class="btn btn-danger btn-sm btn-icon icon-left">
										删除
									</a>
...
function delete_all(){
    ...
    $.ajax({
        ...
        'data':'id='+id,
        'dataType':'json',
        'url':"<?php echo U('Sys/delete_all_link');?>",
        'success':function(dat){
            if(dat.info==1){
                bootbox.alert('删除成功');
                var url="<?php echo U('Sys/link_list');?>";
