...
<script>
    $('#good').jqGrid({
      ...
      /*      {label: '品牌名称', editable: true, search: true, name: 'brand_name', index: 'brand_id', width: 150, align: 'center', stype: 'select', searchoptions: {
                    dataUrl: "<?php echo U('Goods/bselect');?>"}},*/
            ...
            {label: '上架', search: false, name: 'is_on_sale', index: 'is_on_sale', width: 50, align: 'center', formatter: function (a, b, c) {
                    if (a == 0)
                        return '<img src="/Public/admin/assets/images/error.png" width="15"/>';
                    if (a == 1)
                        return '<img src="/Public/admin/assets/images/right.png" width="15"/>';
                }},
...
//            html += '<li class=""><a href="#tab' + subgrid_id + '_5_5" data-toggle="tab">商品详情</a></li>';
            ...
            $('#attr' + rowData.goods_id).jqGrid({
                url: "<?php echo U('Goods/attr');?>?id=" + rowData.goods_id,
                datatype: "json",
                colModel: [
...
//                    return a.replace(/\s/g,"");   }},
            ...
            $('#log' + rowData.goods_id).jqGrid({
                url: "<?php echo U('Goods/log');?>?id=" + rowData.goods_id,
                datatype: "json",
                colModel: [
        ...
        onCellSelect: function (rowid, iCol, cellcontent, e) {
            ...
            if (iCol ==5 || iCol == 6 || iCol == 7 ) {
                $.post("<?php echo U('Goods/is_edit');?>?id=" + rowData.goods_id + '&col=' + iCol, function (_dat) {
                    dat = eval('(' + _dat + ')');
                    if (dat.info == 0) {
                        alert(dat.msg);
                    } else {
        ...
        //toolbar: [true,"top"],
        sortorder: "desc",
        editurl: "<?php echo U('Goods/delete');?>",
        caption: "商品管理"
    });
