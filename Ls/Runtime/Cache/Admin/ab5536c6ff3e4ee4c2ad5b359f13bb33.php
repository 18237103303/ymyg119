     ...
     <!-- Removing search and results count filter -->
			<div class="panel panel-default">
				...
				<div class="panel-body">
                    <p style="float: right;margin:-0.7% 1% 0.7% 0.5%;"><input type="text" placeholder="输入地区" name="sous" value="<?php echo ($sous); ?>"/><button name="sous">搜索</button></p>
					<table class="table table-bordered table-striped" id="example">
                        ...
                        <?php if(is_array($row)): foreach($row as $k=>$vo): ?><tr>
                            <td class="xz"><input type="checkbox" name="dx" value="<?php echo ($k+1); ?>"/></td>
                            <td><?php echo ($vo["id"]); ?></td>
                            <td><input type="text" name="dq" value="<?php echo ($vo["dq"]); ?>"/></td>
                            <td><input type="text" name="hd" value="<?php echo ($vo["h_p"]); ?>"/>元</td>
                            <td><input type="text" name="ps" value="<?php echo ($vo["p_p"]); ?>"/>元</td>
                            <td>
                    ...
                    <p style="float: right;margin-right: 1%;"><?php echo ($rpage); ?></p>
				</div>
			</div>
<style>
...
<script>
    $(function(){
        ...
        $('.xg').click(function(){
           var index= $(this).parents('tr').index();
            var zhi1= zhi(index);
                if(zhi1!=-99){
                    //数据处理部分
                    $.post("<?php echo U('zfz/dqps');?>",{xinz:'xinz',zhi2:zhi1},function(data){
                        if(data==1){
                            bootbox.alert('修改成功');
                        }else{
                            bootbox.alert('修改失败');
        ...
        $('.sc').click(function(){
            var index= $(this).parents('tr').index();
            $.post("<?php echo U('zfz/dqps');?>",{xinz:'sc',index:index},function(data){
             if(data==1){
                 bootbox.alert('删除成功');
        ...
        $('.xg1').click(function(){
            ...
            var zhi3=zhi2.join('|');
            $.post("<?php echo U('zfz/dqps');?>",{xinz:'xg1',zhi2:zhi3},function(data){
                if(data==1){
                    bootbox.alert('修改成功');
                }else{
                    bootbox.alert('修改失败');
        ...
        $('.sc1').click(function(){
                ...
                if(len<1){
                    ...
                    $.post("<?php echo U('zfz/dqps');?>",{xinz:'sc1',zhi2:idd},function(data){
                        if(data==1){
                            //bootbox.alert('删除成功');
                           var ls= idd1.split('|');
                            alert(ls);
