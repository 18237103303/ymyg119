...
<script>  
  function fenlei_delmore(obj,cName){
	  ...
	  for(var $i=0;$i<checkall.length;$i++){
			...
			<div class="panel panel-default">
				<div class="panel-body">
				<form name="myform" action="<?php echo U('Goods/gtypedeleteall');?>" method="post">
					<table class="table table-bordered table-striped" id="example-2">
						...
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
								<td>
								<input type="checkbox"  name="cName[]" value="<?php echo ($list['type_id']); ?>">
								</td>
								<td><?php echo ($list["type_name"]); ?></td>
								<?php $where['cat_id']=$list['type_id']; $resultcount=M('goods_attribute')->where($where)->select(); $count=count($resultcount); ?>
								<td><?php echo ($count); ?></td>
								
								<td>
                                                                
									<a href="#" class="btn btn-secondary btn-sm btn-icon icon-left" onclick="edit(<?php echo ($list["type_id"]); ?>,this)">
										编辑
									</a>
									
									<a href="<?php echo U('Goodstype/gtypedelete');?>?id=<?php echo ($list["type_id"]); ?>" class="btn btn-danger btn-sm btn-icon icon-left" onclick="return confirm('确定要删除吗？');">
										删除
									</a>
                                                               
									<a href="<?php echo U('Goodstype/gtypeatt');?>?id=<?php echo ($list["type_id"]); ?>" class="btn btn-warning btn-sm btn-icon icon-left">
										属性列表
									</a> 
								</td>
					...
					<?php if(quanx('adattr',2)): ?><button class="btn btn-secondary" data-original-title="点击确定将删除您所选内容" title="" data-placement="bottom" data-toggle="tooltip" type="submit">批量删除</button><?php endif; ?>
					</form> 
					<?php echo ($page); ?>
                ...
                success: {
					...
					if($('#cate_input').val()==""){
                        ...
                        $.ajax({
                            ...
                            'dataType':"json",
                            'url':"<?php echo U('Goodstype/gtypedoadd');?>",																					
                            'success':function(dat){
                              // var data = eval('(' + dat + ')');
							  
                                if(dat.status==1){
                                    //bootbox.alert('添加成功');
                                    var url="<?php echo U('Goodstype/gtype');?>";
                ...
                success: {
                    ...
                    callback: function() {
                        $.ajax({
                            ...
                            'dataType':"json",
                            'url':"<?php echo U('Goodstype/gtypeupdate');?>",
                            'success':function(dat){
                                if(dat.status==1){
                                    //bootbox.alert('编辑成功');
                                    var url="<?php echo U('Goodstype/gtype');?>";
